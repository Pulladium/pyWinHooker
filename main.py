...
class MouseTracker:
    ...
    def on_mouse_left_down(self, event):
        try:
            ...
            self.start_mouse_pos = (event.Position.x, event.Position.y)
            return True
    ...
    def on_mouse_left_up(self, event):
        try:
            ...
            self.end_mouse_pos = (event.Position.x, event.Position.y)
...
async def main():
    ...
    mouse_tracking_thread = threading.Thread(target=mouse_tracker.start_mouse_tracking)
    mouse_tracking_thread.daemon = True
    mouse_tracking_thread.start()

    # Запуск асинхронной функции print_selected_text
    await print_selected_text(mouse_tracker.signal_queue)
